...
contract MarketPlace is IMarketPlace {
    // Custom Errors
    error MarketPlace__OnlyTokenOwnerCanListToken(address _owner, address _from);
    ...
    mapping(address erc721Address => mapping(uint256 tokenId => address tokenOwner)) private s_tokenOwner;
    ...
    mapping(address erc721Addresses => NftsModels[] listingDetails) private s_listedNftsCollections; // This keep tracks of all listed Nft under a specific collection
    ...
    address i_owner;
    mapping(address => uint256) s_earnedPercentage;

    // Modifiers

    // This modifier checks if caller is owner of the token they want to unlist
    modifier checkOwerOfToken(address _from, address _erc721Address, uint256 _tokenId) {
    ...
    modifier checkAmount(uint256 _amount, uint256 balance) {
        if (_amount > balance) {
            revert MarketPlace__NotEnoughBalance();
        }
        _;
    }

    modifier checkOnlyOwner(address _caller) {
        if (_caller != i_owner) {
    ...
    function listNFT(address _erc721ContractAddress, uint256 _erc721TokenId, uint256 _listingAmount) external {
        _listNFT(_erc721ContractAddress, _erc721TokenId, _listingAmount, msg.sender);
        IERC721(_erc721ContractAddress).safeTransferFrom(msg.sender, i_nftHolderContract, _erc721TokenId, "");
    }
     ...
     */
    function unListNft(address _erc721ContractAddress, uint256 _erc721TokenId)
        external
        checkOwerOfToken(msg.sender, _erc721ContractAddress, _erc721TokenId)
    {
        ...
        bool isSuccess =
            NFTHolderContract(i_nftHolderContract).transferToken(msg.sender, _erc721ContractAddress, _erc721TokenId);
        if (!isSuccess) {
            revert MarketPlace__TransferFailed();
        }
    }

    function buyNft(address _erc721Address, uint256 _index, uint256 _amount) external {
        NftsModels memory _nftModel = s_listedNftsCollections[_erc721Address][_index];
        s_userBalance[_nftModel.owner] += _amount - _percentageCalculator(_amount);
        s_earnedPercentage[i_owner] += _percentageCalculator(_amount);

        if (_amount < _nftModel.listingAmount) {
            revert MarketPlace__AmountLessThanSaleValue();
        }
        _removeFromUserList(_nftModel.owner, _erc721Address, _nftModel.erc721TokenId);
        ...
        bool isSucessful = IERC20(i_wrappedEthAddress).transferFrom(msg.sender, i_nftHolderContract, _amount);

        if (!isSucessful) {
            revert MarketPlace__TransferFailed();
        }
        bool isTransfered =
            NFTHolderContract(i_nftHolderContract).transferToken(msg.sender, _erc721Address, _nftModel.erc721TokenId);
        if (!isTransfered) {
            revert MarketPlace__TransferFailed();
        }
    }
    ...
    function withdraw(uint256 _amount)
        external
        checkZeroWithdraw(_amount)
        checkAmount(_amount, s_userBalance[msg.sender])
    {
        s_userBalance[msg.sender] -= _amount;
        bool isSuccessful = NFTHolderContract(i_nftHolderContract).withdrawEth(_amount, i_wrappedEthAddress, msg.sender);
        if (!isSuccessful) {
            revert MarketPlace__TransferFailed();
        }
    }

    function withdrawEarnedPercentage(uint256 _amount)
        external
        checkZeroWithdraw(_amount)
        checkAmount(_amount, s_earnedPercentage[i_owner])
        checkOnlyOwner(msg.sender)
    {
        s_earnedPercentage[i_owner] -= _amount;
        bool isSuccessful = NFTHolderContract(i_nftHolderContract).withdrawEth(_amount, i_wrappedEthAddress, msg.sender);
        if (!isSuccessful) {
            revert MarketPlace__TransferFailed();
        }
    ...
    function _listNFT(address _erc721ContractAddress, uint256 _erc721TokenId, uint256 _listingAmount, address _from)
        private
    {
        address tokenOwner = IERC721(_erc721ContractAddress).ownerOf(_erc721TokenId);
        if (_from != tokenOwner) {
    ...
    function _removeFromUserList(address _owner, address _erc721Address, uint256 _tokenId) private {
        NftsModels[] memory userNfts = s_userListedNFts[_owner];
        for (uint256 index = 0; index < userNfts.length; index++) {
            if (userNfts[index].erc721TokenId == _tokenId && userNfts[index].erc721Address == _erc721Address) {
                ...
                if (userNfts.length > 1) {
                    ...
                    s_userListedNFts[_owner][index] = s_userListedNFts[_owner][userNfts.length - 1];
    ...
    function _removeFromListedCollection(address _erc721Address, uint256 tokenId) private {
        uint256 length = s_listedNftsCollections[_erc721Address].length;
        for (uint256 index = 0; index < length; index++) {
            if (s_listedNftsCollections[_erc721Address][index].erc721TokenId == tokenId) {
                delete s_listedNftsCollections[_erc721Address][index];
                if (length > 1) {
                    s_listedNftsCollections[_erc721Address][index] = s_listedNftsCollections[_erc721Address][length - 1];
    ...
    function _percentageCalculator(uint256 _amount) public pure returns (uint256) {
        return (_amount * 1) / 100;
    }
    // View Functions

    function getUserListedNfts(address _user) public view returns (NftsModels[] memory) {
    ...
    function getNftBalanceOf(address _erc721Address) public view returns (uint256) {
        return IERC721(_erc721Address).balanceOf(i_nftHolderContract);
    }

    function getTokensListedInCollections(address erc721CollectionContractAddress)
        public
        view
        returns (NftsModels[] memory)
    {
        return s_listedNftsCollections[erc721CollectionContractAddress];
    }

    function getListedCollectionAddress() public view returns (address[] memory) {
        return s_erc721Addresses;
    }

    function getTokenOwner(address _erc721Address, uint256 tokenId) public view returns (address) {
        return s_tokenOwner[_erc721Address][tokenId];
    }

    function getEarnedPercentage() public view returns (uint256) {
        return s_earnedPercentage[i_owner];
    }
}
